...
function populateStates( countryElementId, stateElementId, config ){
    var state = config['state'] || 'Select State';
    var stateId = config['state'] || '-1';
    var countryId = config['country'] || 0;

    var selectedCountryIndex = country_arr.indexOf(countryId) + 1 || document.getElementById( countryElementId ).selectedIndex;
    var stateElement = document.getElementById( stateElementId );
    stateElement.length=0;	// Fixed by Julian Woods
    stateElement.options[0] = new Option(state, stateId);

    var state_arr = s_a[selectedCountryIndex].split("|");
    stateElement.selectedIndex = state_arr.indexOf(state) + 1;
...
function populateCountries(countryElementId, stateElementId, config){
    var country = config['country'] || 'Select Country';
    var countryId = config['country'] || '-1';

    var countryElement = document.getElementById(countryElementId);
    countryElement.length=0;
    countryElement.options[0] = new Option(country, countryId);
    countryElement.selectedIndex = country_arr.indexOf(country) + 1;
    for (var i=0; i<country_arr.length; i++) {
        countryElement.options[countryElement.length] = new Option(country_arr[i],country_arr[i]);
    }

    populateStates( countryElementId, stateElementId, config);
